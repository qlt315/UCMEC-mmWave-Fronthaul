...
class MA_MPO_dyna_coop(object):
    def __init__(self, render: bool = False):
        ...
        self.obs_dim = 4 + (self.M_sim - 1) * 2  # set the observation dimension of agents
        self.action_dim = 4
        self._render = render
        ...
        # r in [0, 10e8]
        self.obs_low = np.array([50000, 500, 0, 0])
        self.obs_high = np.array([100000, 1000, 2, 0.2])
    ...
    def uplink_rate_cal(self, p, omega, cluster_matrix, theta):  # calculate the uplink transmit rate in Eq. (12)
            ...
            if omega[i] == 0:  # Local processing
                ...
                for j in range(self.N_sim):
                    if cluster_matrix[i, j] == 1:
                        ...
                        SINR_access_noise[i, 0] = self.noise_access * theta[i, j]
                ...
                for k in range(self.M_sim):
                    if k == i or omega[k] == 0:
                        ...
                        for j in range(self.N_sim):
                            if cluster_matrix[i, j] == 1:
                                SINR_access_inter[i, 0] = SINR_access_inter[i, 0] + theta[i, j] * self.beta[k, j] * \
                                                          p[k]
    ...
    def step(self, action):
        self.step_num += 1
        # task parameter
        Task_size = np.random.uniform(50000, 100000, [1, self.M])  # task size in bit
        ...
        for i in range(self.M_sim):
            if omega_current[i] == 0:
                local_delay[i, 0] = Task_density[0, i] * Task_size[0, i] / self.C_user[0, i]
        ...
        print("Average Total Delay:", np.sum(total_delay) / self.M_sim)
        ...
        for i in range(self.agent_num):
            if self.step_num == 1:
                observation[i, 0] = Task_size[0, i]
                observation[i, 1] = Task_density[0, i]
            else:
                observation[i, 0] = Task_size[0, i]
                observation[i, 1] = Task_density[0, i]
                observation[i, 2] = self.omega_last[i]
                observation[i, 3] = self.delay_last[i, 0]
                for j in range(self.agent_num - 1):
                    observation[i, (4 + j * 2):(4 + (j + 1) * 2)] = np.array(
