...
class MA_CBO_stat_noncoop(object):
    def __init__(self, render: bool = False):
        ...
        self.P_max = 0.3  # maximum transmit power of user / pilot power
        ...
        if self.is_mobile:
            ...
            for i in range(self.M):
                for j in range(self.N):
                    ...
                    self.beta[i, j] = pow(10, self.PL[i, j] / 10) * pow(10, (self.sigma_s * self.mu[i, j]) / 10)
                    for k in range(self.varsig):
                        self.h[i, j, k] = np.random.normal(loc=0, scale=0.5) + 1j * np.random.normal(loc=0,
                                                                                                     scale=0.5)
                        self.access_chan[i, j, k] = np.sqrt(self.beta[i, j]) * self.h[i, j, k]
