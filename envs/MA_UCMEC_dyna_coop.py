...
class MA_UCMEC_dyna_coop(object):
    def __init__(self, render: bool = False):
        ...
        # a in {0,1,2,3}, p in {0, 1, 2, 3} (totally 5 levels (p+1)/5*100 mW)
        ...
        self.obs_high = np.array([100000, 1000, 2, self.P_max, 0.2])
    ...
    def step(self, action):
        ...
        for i in range(self.agent_num):
            if self.step_num == 1:
                ...
                observation[i, 3] = self.p_last[i]
                observation[i, 4] = self.delay_last[i, 0]
                for j in range(self.agent_num - 1):
                    observation[i, (5 + j * 3):(5 + (j + 1) * 3)] = np.array(
